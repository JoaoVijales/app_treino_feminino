...
import { Activity, Home, BarChart3, Settings, ChevronRight, Play, Check, X, Calendar, Zap, Moon, Sun, Droplet, Heart, TrendingUp, Clock, Target, Info, ChevronLeft, Pause, LucideProps } from 'lucide-react';
            ...
            <div className="space-y-4">
              {screen.type === 'text' && (
                <input
                  ...
                  value={userData[screen.field as keyof UserData] as string}
                  ...
                  {screen.options?.map((option) => (
                    <button
                      ...
                      className={`w-full p-4 rounded-2xl border-2 transition-all text-left flex items-center gap-4 ${
                        userData[screen.field as keyof UserData] === option.value
                ...
                <div className="space-y-3">
                  {screen.options?.map((option) => (
                    <button
                      key={option.value}
                      onClick={() => {
                        const current = userData[screen.field as keyof UserData] || [];
                        const newValue = (current as string[]).includes(option.value)
                          ? (current as string[]).filter((v: string) => v !== option.value)
                          : [...(current as string[]), option.value];
                        setUserData({...userData, [screen.field]: newValue});
                      }}
                      className={`w-full p-4 rounded-2xl border-2 transition-all text-left flex items-center gap-4 ${
                        (userData[screen.field as keyof UserData] as string[]).includes(option.value)
                          ? 'border-rose-400 bg-rose-50'
                          : 'border-gray-200 hover:border-gray-300'
                      }`}
                    >
